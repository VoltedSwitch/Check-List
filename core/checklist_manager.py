...
class ChecklistsManager:
    ...
    def reposition_checklist(
        self, checklist_number: int, insertion_position: int
    ) -> None:
        checklist: Checklist = self.remove_checklist(
            checklist_number, return_removed_checklist=True
        )
        self.insert_checklist(insertion_position, checklist)

    def remove_checklist(
        self, checklist_number: int, return_removed_checklist: bool = False
    ) -> None | Checklist:
    ...
    def is_valid_checklist_number_str(self, checklist_number: str) -> bool:
        return checklist_number.isdigit() and (
            0 <= int(checklist_number) - 1 < len(self.user_checklists)
        )
